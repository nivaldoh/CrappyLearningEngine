...
VulkanWrapper::VulkanWrapper() {
    // GLFW instance needs to be set first
    // Initialize();
}
...
int VulkanWrapper::DeviceWaitIdle() {
...
SwapChainSupportDetails VulkanWrapper::QuerySwapChainSupport(VkPhysicalDevice device) {
...
std::string UtilGetRelativePath(const std::string& targetFile, int levels) {
    ...
    for (int i = 0; i < levels; ++i) {
        if (exePath.has_parent_path()) {
            ...
            throw std::runtime_error("Reached the root directory");
        }
    }
